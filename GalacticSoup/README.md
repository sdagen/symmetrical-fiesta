...
| `behavior/` | Behavioral component library (model references + subsystem references), per-variant plant models, data dictionaries, build scripts, and unit tests. See [`docs/09_behavioral_models.md`](docs/09_behavioral_models.md) / [`docs/10_behavioral_trade_update.md`](docs/10_behavioral_trade_update.md). |
...
| `behavior/components/Beh{Storage,PrepUnit,CookLine,CookVat,QCStation,Packager,Supervisor,ProductionCell}.slx` | Behavioral | Eight model references forming the shared behavioral component library — one per recurring production role (storage, prep, cooking, batch cooking + Stateflow/Simscape, QC, packaging, supervision, cell composition). See [`docs/09_behavioral_models.md`](docs/09_behavioral_models.md) §1. |
...
| `behavior/components/BehKettleBehavior.slx` | Behavioral | Adapter behavior model demonstrating port-preserving architecture integration for the LeanBroth kettles — not linked in the committed baseline (see ADR-017). |
| `behavior/plants/BehPlant{HyperCook,LeanBroth,IronLadle}.slx` | Behavioral | Top-level behavioral plant models, one per physical variant, composing the component library per variant topology and parameters. |
| `behavior/data/*.sldd` | Behavioral | Five data dictionaries: `BehaviorInterfaces` (shared types), `BehParamsCommon` (physics constants), and `BehParams{HyperCook,LeanBroth,IronLadle}` (per-variant instance parameters, each referencing Common). |
...
For LeanBroth this is decisive: simulated steady throughput (196.6 bph) falls below the SR-GS-002 200 bph floor, and the formal Requirements Table gate flags exactly the Throughput row. LeanBroth is now excluded from MCDA scoring and loses its status as the documented CostLean descope option pending a QC or prep redesign. With LeanBroth excluded, **IronLadle wins all four stakeholder weighting scenarios** (Balanced, ThroughputFirst, CostLean, and MissionAssurance) and **98.4%** of a 5,000-sample Monte Carlo random-weight sensitivity sweep against HyperCook's 1.6% — up from 3 of 4 scenarios and an 84% win share in the static analysis. IronLadle's single-fault resilience, previously an idealized 66.7% capacity estimate, is now demonstrated dynamically: fault injection steps it down to roughly two-thirds output with the BehSupervisor chart transitioning to a Degraded mode, while HyperCook and LeanBroth both collapse to zero as single-string topologies. IronLadle's energy per bowl (1.212 kWh) lands mid-field between LeanBroth's batch efficiency (0.815 kWh) and HyperCook's continuous draw (1.556 kWh); a new finding is that both batch-plant variants take on the order of an hour to reach steady output from a cold start, versus 149 s for HyperCook, favoring HyperCook specifically for surge-restart scenarios.

**Recommendation: adopt IronLadle as the baseline physical architecture — reinforced, not revised** (see [`docs/07_decision_log.md`](docs/07_decision_log.md) ADR-009, ADR-018). The three original follow-up actions stand (negotiate IronLadle's thin cost margin, define a degraded-mode operations procedure for the single-cell-loss contingency, and — pending its QC/prep redesign — reconsider LeanBroth as a descope option), joined by two new ones from the behavioral update: a LeanBroth QC redesign study, and backpressure modeling to address the current gap where buffer overflow discards flow rather than conserving mass.

![Worst-case fault response — HyperCook and LeanBroth collapse to zero, IronLadle steps down to two-thirds and enters Degraded mode](docs/figures/behavioral_fault.png)
