...
Artifacts: [`../tests/system/GalacticSoupSystemTests.mldatx`](../tests/system/GalacticSoupSystemTests.mldatx) (generated by [`../tests/system/buildSystemTestFile.m`](../tests/system/buildSystemTestFile.m)), the remaining MATLAB tiers in [`../tests/`](../tests/) and [`../behavior/tests/`](../behavior/tests/) (now `sltest.TestCase` subclasses), [`../tests/runAllTests.m`](../tests/runAllTests.m). Decision: ADR-022 in [`07_decision_log.md`](07_decision_log.md).
...
`tSystemNominal.m` and `tSystemFault.m` are retired as redundant now that [`../tests/system/GalacticSoupSystemTests.mldatx`](../tests/system/GalacticSoupSystemTests.mldatx) covers the same six simulations with the same baselined figures, plus the links `matlab.unittest` could never carry.

## 2. The `.mldatx` design

[`buildSystemTestFile.m`](../tests/system/buildSystemTestFile.m) generates the test file the same way [`buildComplianceGate.m`](../analysis/pipeline/buildComplianceGate.m) generates the gate model (ADR-010): destructively and idempotently, never hand-edited. It purges stale `Verify` links to the artifact from the requirement set, deletes any existing `.mldatx`, and rebuilds from a literal table of cases in the script — the same golden-value-as-literal philosophy ADR-021 established for the MATLAB tiers.
