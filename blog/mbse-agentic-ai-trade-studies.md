...
Requirements verified by executed simulation stand at nine of 28 for EverSimmer, eight for HyperCook, and two for LeanBroth — counted per candidate architecture, because an aggregate across three mutually exclusive designs proved worse than no status at all: it showed the gravity requirement verified while the trade winner was exactly the variant failing it. The suite has grown to 72 tests, every verified requirement has its own design-space sweep, and every sweep paid for itself with either a margin number nobody had or a finding nobody expected.
